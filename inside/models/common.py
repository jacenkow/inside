...
class ProjectModelAttention(ProjectModel):
    """Abstract class for the project-related models with attentions."""
    def __init__(self):
        super().__init__()

    def attention(self, x):
        """Return attention maps."""
        return self.encoder_call(x)[0]

    def encoder_call(self, x):
        z = self.encoder(x['x'])
        a, z = self.conditioning([z, x['z']])
